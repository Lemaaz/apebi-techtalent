...
const AVATAR_PALETTES = [
  'bg-[#00AFD2]/15 text-[#00AFD2]',
  'bg-[#3A4652]/15 text-[#8b98a5]',
  ...
  return (
    <div className="min-h-dvh bg-[#0F0F0F] text-white">
      ...
      <header className="border-b border-white/8">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3 sm:px-6">
          <Link href="/" className="font-heading text-[15px] font-bold">
            APEBI <span className="text-[#00AFD2]">Tech</span>Talent
          </Link>
          <Link
            href="/offres"
            className="rounded-lg border border-white/15 px-3 py-1.5 text-[12px] font-semibold text-white/70 transition-colors hover:border-[#00AFD2]/40 hover:text-[#00AFD2]"
          >
            Voir les offres
          </Link>
        </div>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {talent.has_techtalent_label && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#00AFD2]/15 px-2.5 py-0.5 text-[11px] font-semibold text-[#00AFD2]">
          ...
          <div className="flex gap-2">
            ...
            {talent.portfolio_url && (
              <a href={talent.portfolio_url} target="_blank" rel="noopener noreferrer"
                 className="inline-flex size-9 items-center justify-center rounded-lg border border-white/15 text-white/50 transition-colors hover:text-[#00AFD2]" aria-label="Portfolio">
              ...
              {[...skillsByDomain.values()].map(({ domainName, skills }) => (
                <div key={domainName}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {skills.map((name) => (
                      <span key={name} className="rounded-full border border-[#00AFD2]/20 bg-[#00AFD2]/5 px-2.5 py-1 text-xs font-medium text-[#00AFD2]">{name}</span>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <ul className="space-y-5" role="list">
              {experiences.map((exp) => (
                <li key={exp.id} className="relative border-l-2 border-white/10 pl-4">
                  {exp.is_current && <div aria-hidden className="absolute -left-1 top-1.5 size-2 rounded-full bg-[#00AFD2]" />}
        ...
        {/* ── CTA conversion ── */}
        <section
          className="mt-12 rounded-2xl border border-[#00AFD2]/20 p-6 text-center"
        ...
        >
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-3">
            <Link href="/entreprises/inscription" className="inline-flex items-center gap-1.5 rounded-lg bg-[#00AFD2] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#0096B5]">
        ...
        <p className="mt-6 text-center text-[11px] text-white/30">
          Profil public partagé via APEBI TechTalent · <Link href="/" className="text-[#00AFD2] hover:underline">apebi-techtalent</Link>
        </p>
      </main>
    </div>
  )
